...
public abstract class PropertiesBase
{
    ...
    public static <T extends PropertiesBase> T reload(T properties, Class<T> propertiesClass)
    {
        ...
        try
        {
            ...
            if (sourceChanged)
            {
                Journeymap.getLogger().info("Loaded {} from {}", propertiesClass.getSimpleName(), reloadedProperties.getFile());
            }
    ...
    public boolean save()
    {
        synchronized (gson)
        {
            ...
            try
            {
                ...
                if (!propFile.exists())
                {
                    Journeymap.getLogger().info("Creating config file: {}", propFile);
                ...
                {
                    Journeymap.getLogger().info("Updating config file from version \"{}\" to \"{}\": {}", configVersion, Journeymap.JM_VERSION, propFile);
            ...
            {
                Journeymap.getLogger().error("Can't save config file {}: {}", propFile, LogFormatter.toString(e));
                return false;
            }
        }
    }
    ...
    public <T extends PropertiesBase> T load()
    {
        ...
        try
        {
            if (propFile.canRead())
            {
                ...
                if (saveNeeded)
                {
                    Journeymap.getLogger().info("Config file needs to be updated: {}", propFile.getName());
                }
            }
            else
            {
        ...
        {
            Journeymap.getLogger().error("Can't load config file {}: {}", propFile, e.getMessage());
            ...
            {
                Journeymap.getLogger().error("Can't rename config file {}: {}", propFile, e3.getMessage());
            }
